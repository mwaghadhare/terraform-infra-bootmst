...
# to decrypt keys
resource "aws_iam_role" "ep-terminator-key-access" {
  ...
  assume_role_policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      "Effect": "Allow",
      "Principal": {
        "AWS": "${aws_iam_role.ep-terminator_iam_role.arn}"
      },
      "Action": "sts:AssumeRole"
    }
  ]
...
resource "aws_iam_policy" "ep-terminator_key_access_policy" {
  name        = "${var.org}-${var.env}-${var.name}-key-access"
  ...
  policy = <<EOF
{
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Resource": [
                "arn:aws:ssm:${var.region}:8888xxxxx:parameter/my/${var.env}/ep-terminator*"
            ]
        },
        {
...
resource "aws_iam_role_policy_attachment" "ep-terminator_key_access_policy_attach" {
  role       = "${aws_iam_role.ep-terminator-key-access.name}"
  policy_arn = "${aws_iam_policy.ep-terminator_key_access_policy.arn}"
}

# Leaving this here for clarity, if this is in module, you want to write
# out the arn of the key access role
output "ep-terminator-key-access" {
