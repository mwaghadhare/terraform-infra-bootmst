...
resource "aws_iam_instance_profile" "ep-terminator_iam_profile" {
  count = "${var.instance_profile == "" ? 1 : 0}"
  name  = "${var.org}-${var.env}-${var.name}-iam"
  role  = "${aws_iam_role.ep-terminator_iam_role.name}"
}

resource "aws_iam_role" "ep-terminator_iam_role" {
  count = "${var.instance_profile == "" ? 1 : 0}"
...
resource "aws_iam_policy" "ep-terminator_iam_policy" {
  count       = "${var.instance_profile == "" ? 1 : 0}"
  name        = "${var.org}-${var.env}-${var.name}"
...
resource "aws_iam_role_policy_attachment" "ep-terminator_policy_attach" {
  count      = "${var.instance_profile == "" ? 1 : 0}"
  role       = "${aws_iam_role.ep-terminator_iam_role.name}"
  policy_arn = "${aws_iam_policy.ep-terminator_iam_policy.arn}"
}
