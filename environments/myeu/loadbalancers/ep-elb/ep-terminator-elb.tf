module "ep-terminator-elb" {
  source = "../../../../modules/loadbalancers/ep-elb/"

  name                        = "${var.name}"
  route53_record              = "${var.route53_record}"
  route53_record_public       = "${var.route53_record_public}"
  subnets                     = ["${data.terraform_remote_state.vpc.public_subnets}"]
  security_groups             = ["${data.terraform_remote_state.vpc.ep-terminator_elb_sg_id}"]
  ...
  instances                   = ["${split(",", data.terraform_remote_state.ep-terminator.ep-terminator_instance_ids)}"]
