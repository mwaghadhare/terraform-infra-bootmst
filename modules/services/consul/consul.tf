resource "aws_instance" "consul" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.consul_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-consul" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "role[${var.role_name}]", "my_consul_register::consului","diamond::consul"]
    ...
    connection {
      ...
      host                = "${aws_instance.consul.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-consul-1" {
  depends_on = ["null_resource.chef-bootstrap-consul"]
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.consul.*.private_ip,count.index + 1)}"
    }
  }
}
