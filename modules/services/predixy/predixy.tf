resource "aws_instance" "predixy" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.predixy_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-predixy" {
  count = "${var.number_of_instances}"
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      host                = "${aws_instance.predixy.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-predixy-1" {
  depends_on = ["null_resource.chef-bootstrap-predixy"]
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.predixy.*.private_ip,count.index + 1)}"
    }
  }
}