chef-server Terraform module
========================

Terraform module which creates AWS Ec2 machines and install chef-server 
...
```
module "chef-server" {
  source                     = "../../../../modules/services/chef-server/"
  key_name                   = "${var.env}-${var.region}"
  security_groups            = ["${data.terraform_remote_state.vpc.chef-server_sg_id}"]
  org                        = "${var.org}"
  region                     = "${var.region}"
  fullaz                     = "${var.region}b"
  env                        = "${var.env}"
  ...
  chef-servering             = "${var.chef-servering}"
}
```
