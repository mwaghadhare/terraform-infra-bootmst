resource "aws_cloudwatch_metric_alarm" "kafka_status_check" {
  alarm_name          = "StatusCheck-kafka-${format("%03d",count.index)}-${var.env}"
  ...
  dimensions {
    InstanceId = "${element(aws_instance.kafka.*.id, count.index)}"
  }
...
resource "aws_cloudwatch_metric_alarm" "kafka" {
  alarm_name          = "CPUUtilization-kafka-${format("%03d",count.index)}-${var.env}"
  comparison_operator = "${var.comparison_operator}"
  evaluation_periods  = "${var.evaluation_periods}"
