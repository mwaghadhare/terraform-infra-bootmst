resource "aws_instance" "kafka" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.kafka_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-kafka" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "my_kafka::default","diamond::kafka", "role[consul_agent]","my_consul_register::kafka"]
    ...
    connection {
      ...
      host                = "${aws_instance.kafka.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-kafka-1" {
  depends_on = ["null_resource.chef-bootstrap-kafka"]
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.kafka.*.private_ip,count.index + 1)}"
    }
  }
}
