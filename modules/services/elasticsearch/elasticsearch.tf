resource "aws_instance" "elasticsearch" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.elasticsearch_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-elasticsearch" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "role[es-master]","diamond::elasticsearch-master"]
    ...
    connection {
      ...
      host                = "${aws_instance.elasticsearch.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-elasticsearch-1" {
  depends_on = ["null_resource.chef-bootstrap-elasticsearch"]
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.elasticsearch.*.private_ip,count.index + 1)}"
    }
  }
}
