resource "aws_cloudwatch_metric_alarm" "monitor_status_check" {
  alarm_name          = "StatusCheck-monitor-${format("%03d",count.index)}-${var.env}"
  ...
  dimensions {
    InstanceId = "${element(aws_instance.monitor.*.id, count.index)}"
  }
...
resource "aws_cloudwatch_metric_alarm" "monitor" {
  alarm_name          = "CPUUtilization-monitor-${format("%03d",count.index)}-${var.env}"
  comparison_operator = "${var.comparison_operator}"
  evaluation_periods  = "${var.evaluation_periods}"
