...
resource "aws_iam_instance_profile" "monitor_iam_profile" {
  count = "${var.instance_profile == "" ? 1 : 0}"
  name  = "${var.org}-${var.env}-${var.name}-iam"
  role  = "${aws_iam_role.monitor_iam_role.name}"
}

resource "aws_iam_role" "monitor_iam_role" {
  count = "${var.instance_profile == "" ? 1 : 0}"
...
resource "aws_iam_policy" "monitor_iam_policy" {
  count       = "${var.instance_profile == "" ? 1 : 0}"
  name        = "${var.org}-${var.env}-${var.name}"
...
resource "aws_iam_role_policy_attachment" "monitor_policy_attach" {
  count      = "${var.instance_profile == "" ? 1 : 0}"
  role       = "${aws_iam_role.monitor_iam_role.name}"
  policy_arn = "${aws_iam_policy.monitor_iam_policy.arn}"
}
