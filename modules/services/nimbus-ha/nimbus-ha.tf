resource "aws_instance" "nimbus-ha" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.nimbus-ha_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-nimbus-ha" {
  count = "${var.number_of_instances}"

  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "role[${var.role_name}]"]
    ...
    connection {
      ...
      host                = "${element(aws_instance.nimbus-ha.*.private_ip,count.index)}"
    }
  }
}
