resource "aws_instance" "nimbus" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.nimbus_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-nimbus" {
  count = "${var.number_of_instances}"

  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "role[${var.role_name}]","diamond::nimbus"]
    ...
    connection {
      ...
      host                = "${aws_instance.nimbus.0.private_ip}"
    }
  }
}
