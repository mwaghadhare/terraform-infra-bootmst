...
aws s3 cp --region ${region}  s3://my-${env}-info/storm-bootstrap/bootstrap_storm_${env}_ha.sh /var/scripts/bootstrap_storm_${env}_ha.sh && chmod +x /var/scripts/bootstrap_storm_${env}_ha.sh
sleep 10
sudo /var/scripts/bootstrap_storm_${env}_ha.sh
sleep 10
sudo chown -R root:root /opt/storm
chmod 755 /tmp/update_hosts_file_ha.py
sleep 20
sudo python /tmp/update_hosts_file_ha.py --env=${env} --region=${region} --domain=${domain} --private_domain=${private_domain} --public_hosted_zone_id=${zone_id_public} --private_hosted_zone_id=${zone_id_private}
