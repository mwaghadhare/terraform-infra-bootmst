...
resource "aws_spot_fleet_request" "mesos-agents_a" {
  ...
  launch_specification {
    ...
    availability_zone      = "${element(split(",", var.fullaz), count.index)}"
    vpc_security_group_ids = ["${var.security_group}"]
    spot_price             = "${var.spotprice}"
    subnet_id              = "${var.subnet_ids[count.index]}"
    weighted_capacity      = "${var.wc-four}"
    user_data              = "${data.template_file.userdata_a.rendered}"
    iam_instance_profile_arn   = "${aws_iam_instance_profile.mesos-agents_iam_profile.arn}"
  ...
  launch_specification {
    ...
    weighted_capacity      = "${var.wc-eight}"
    user_data              = "${data.template_file.userdata_a.rendered}"
    iam_instance_profile   = "${aws_iam_instance_profile.mesos-agents_iam_profile.arn}"
