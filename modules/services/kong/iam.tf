...
resource "aws_iam_instance_profile" "kong_iam_profile" {
...
resource "aws_iam_policy" "kong_iam_policy" {
  count       = "${var.instance_profile == "" ? 1 : 0}"
  name        = "${var.org}-${var.env}-${var.name}"
...
resource "aws_iam_role_policy_attachment" "kong_policy_attach" {
  count      = "${var.instance_profile == "" ? 1 : 0}"
  role       = "${aws_iam_role.kong_iam_role.name}"
  policy_arn = "${aws_iam_policy.kong_iam_policy.arn}"
}
