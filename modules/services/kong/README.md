kong Terraform module
========================

Terraform module which creates AWS Ec2 machines and install  kong.
...
```
module "kong" {
  source                     = "../../../../modules/services/kong/"
  key_name                   = "${var.env}-${var.region}"
  security_groups            = ["${data.terraform_remote_state.vpc.kong_sg_id}"]
  org                        = "${var.org}"
  region                     = "${var.region}"
  fullaz                     = "${var.region}b"
  env                        = "${var.env}"
