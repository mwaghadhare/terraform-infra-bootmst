resource "aws_instance" "kong" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.kong_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-kong" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "role[${var.role_name}]","my_kong::rest_${var.env}","diamond::kong"]
    ...
    connection {
      ...
      host                = "${aws_instance.kong.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-kong-1" {
  depends_on = ["null_resource.chef-bootstrap-kong"]
  count      = "${var.number_of_instances - 1}"

  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "role[${var.role_name}]","diamond::kong"]
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.kong.*.private_ip,count.index + 1)}"
    }
  }
}
