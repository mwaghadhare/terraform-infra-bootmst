...
resource "aws_instance" "load-generator" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.load-generator_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-load-generator" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "java::default"]
    ...
    connection {
      ...
      host                = "${aws_instance.load-generator.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-load-generator-1" {
  depends_on = ["null_resource.chef-bootstrap-load-generator"]
  count      = "${var.number_of_instances - 1}"

  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "my_load-generator::default"]
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.load-generator.*.private_ip,count.index + 1)}"
    }
  }
}
