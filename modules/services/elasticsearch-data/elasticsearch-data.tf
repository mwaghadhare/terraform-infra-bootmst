resource "aws_instance" "elasticsearch-data" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.elasticsearch-data_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-elasticsearch-data" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "role[es-data]","diamond::elasticsearch-data"]
    ...
    connection {
      ...
      host                = "${aws_instance.elasticsearch-data.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-elasticsearch-data-1" {
  depends_on = ["null_resource.chef-bootstrap-elasticsearch-data"]
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.elasticsearch-data.*.private_ip,count.index + 1)}"
    }
  }
}
