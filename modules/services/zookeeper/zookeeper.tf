resource "aws_instance" "zookeeper" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.zookeeper_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-zookeeper" {
  provisioner "chef" {
    environment     = "${var.env}"
    ...
    connection {
      ...
      host                = "${aws_instance.zookeeper.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-zookeeper-1" {
  depends_on = ["null_resource.chef-bootstrap-zookeeper"]
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.zookeeper.*.private_ip,count.index + 1)}"
    }
  }
}
