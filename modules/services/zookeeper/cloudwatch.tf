resource "aws_cloudwatch_metric_alarm" "zookeeper_status_check" {
  alarm_name          = "StatusCheck-zookeeper-${format("%03d",count.index)}-${var.env}"
...
resource "aws_cloudwatch_metric_alarm" "zookeeper" {
  alarm_name          = "CPUUtilization-zookeeper-${format("%03d",count.index)}-${var.env}"
  comparison_operator = "${var.comparison_operator}"
  evaluation_periods  = "${var.evaluation_periods}"
