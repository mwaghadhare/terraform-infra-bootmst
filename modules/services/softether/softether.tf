resource "aws_instance" "softether" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.softether_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-softether" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "softether", "softether::client", "softether::server", "softether::config", "softether::user"]
    ...
    connection {
      ...
      host                = "${aws_instance.softether.0.private_ip}"
    }
  }
}
