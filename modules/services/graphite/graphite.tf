resource "aws_instance" "graphite" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.graphite_iam_profile.*.name))}"
...
# We dont need block device for the data mount point
# as we are using i3 instance type NVME disk
#  ebs_block_device {
...
resource "null_resource" "chef-bootstrap-graphite" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "my_graphite::default","diamond::graphite"]
    ...
    connection {
      ...
      host                = "${aws_instance.graphite.0.private_ip}"
    }
  }
}


resource "null_resource" "chef-bootstrap-graphite-1" {
  depends_on = ["null_resource.chef-bootstrap-graphite"]
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.graphite.*.private_ip,count.index + 1)}"
    }
  }
}