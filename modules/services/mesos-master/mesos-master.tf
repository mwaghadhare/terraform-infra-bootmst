resource "aws_instance" "mesos-master" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.mesos-master_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-mesos-master" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "mesos::master", "marathon::default", "marathon::service", "diamond::mesos"]
    ...
    connection {
      ...
      host                = "${aws_instance.mesos-master.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-mesos-master-1" {
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.mesos-master.*.private_ip,count.index + 1)}"
    }
  }
}
