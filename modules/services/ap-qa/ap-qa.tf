resource "aws_instance" "ap-qa" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.ap-qa_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-ap-qa" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb"]
    ...
    connection {
      ...
      host                = "${aws_instance.ap-qa.0.private_ip}"
    }
  }
}

resource "null_resource" "chef-bootstrap-ap-qa-1" {
  depends_on = ["null_resource.chef-bootstrap-ap-qa"]
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      user                = "ubuntu"
      private_key         = "${file("~/.ssh/${var.env}-${var.region}.pem")}"
      host                = "${element(aws_instance.ap-qa.*.private_ip,count.index + 1)}"
    }
  }
}
