...
resource "aws_cloudwatch_metric_alarm" "kong_status_check" {
  alarm_name          = "UnhealthyHostCount-kong-${var.env}"
  ...
  dimensions {
    LoadBalancerName = "${aws_elb.kong.name}"
  }
...
resource "aws_cloudwatch_metric_alarm" "elb_requestcount_check" {
  alarm_name          = "RequestCount-kong-${var.env}"
  comparison_operator = "${var.comparison_operator}"
  evaluation_periods  = "${var.evaluation_periods}"
...
resource "aws_cloudwatch_metric_alarm" "elb_http5xx_check" {
  alarm_name          = "http5xx-kong-${var.env}"
  comparison_operator = "${var.comparison_operator}"
  evaluation_periods  = "${var.evaluation_periods}"
...
resource "aws_cloudwatch_metric_alarm" "elb_latency_check" {
  alarm_name          = "latency-kong-${var.env}"
  comparison_operator = "${var.comparison_operator}"
  evaluation_periods  = "${var.evaluation_periods}"
