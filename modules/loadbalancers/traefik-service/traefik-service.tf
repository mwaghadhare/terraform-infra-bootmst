resource "aws_instance" "traefik-service" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.traefik-service_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-traefik-service" {
  count = "${var.number_of_instances}"
  ...
  provisioner "chef" {
    ...
    connection {
      ...
      host                = "${element(aws_instance.traefik-service.*.private_ip, count.index)}"
    }
  }
}
