...
resource "aws_cloudwatch_metric_alarm" "client-terminator_status_check" {
  alarm_name          = "UnhealthyHostCount-client-terminator-${var.env}"
  ...
  dimensions {
    LoadBalancerName = "${aws_elb.client-terminator.name}"
  }
...
resource "aws_cloudwatch_metric_alarm" "elb_requestcount_check" {
  alarm_name          = "RequestCount-client-terminator-${var.env}"
  comparison_operator = "${var.comparison_operator}"
  evaluation_periods  = "${var.evaluation_periods}"
...
resource "aws_cloudwatch_metric_alarm" "elb_http5xx_check" {
  alarm_name          = "http5xx-client-terminator-${var.env}"
  comparison_operator = "${var.comparison_operator}"
  evaluation_periods  = "${var.evaluation_periods}"
...
resource "aws_cloudwatch_metric_alarm" "elb_latency_check" {
  alarm_name          = "latency-client-terminator-${var.env}"
  comparison_operator = "${var.comparison_operator}"
  evaluation_periods  = "${var.evaluation_periods}"
