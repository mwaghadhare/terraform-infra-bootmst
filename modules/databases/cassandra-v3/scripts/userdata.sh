...
sudo echo /dev/xvdb  /opt ext4 defaults,nofail,noatime 0 2 >> /etc/fstab


sudo mkfs -t ext4 /dev/xvdc
sudo mkdir /data
sudo mount /dev/xvdc /data
sudo echo /dev/xvdc  /data ext4 defaults,nofail,noatime 0 2 >> /etc/fstab
...
sudo echo 'export PATH="$PATH:/opt/cassandra/bin/"' >> /root/.bashrc
