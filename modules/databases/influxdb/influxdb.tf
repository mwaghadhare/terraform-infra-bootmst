resource "aws_instance" "influxdb" {
  ...
  iam_instance_profile   = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.influxdb_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-influxdb" {
  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb", "my_influx::default"]
    ...
    connection {
      ...
      host                = "${aws_instance.influxdb.0.private_ip}"
    }
  }
}
