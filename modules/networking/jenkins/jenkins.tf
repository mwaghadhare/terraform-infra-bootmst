resource "aws_instance" "jenkins" {
  ...
  iam_instance_profile        = "${coalesce(var.instance_profile,join("",aws_iam_instance_profile.jenkins_iam_profile.*.name))}"
...
resource "null_resource" "chef-bootstrap-jenkins" {
  count = "${var.number_of_instances}"

  provisioner "chef" {
    environment     = "${var.env}"
    run_list        = ["my_base_cb","diamond::jenkins"]
    ...
    connection {
      ...
      host        = "${element(aws_instance.jenkins.*.private_ip, count.index)}"
    }
  }
}
